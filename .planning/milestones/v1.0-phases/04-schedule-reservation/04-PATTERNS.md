...
class Notification(
    ...
    val classSession: ClassSession?,      // 휴강 세션당 1건 요약형 대응(D-097)
...
```
`PassTransaction`의 "주체는 항상 admin"이던 제약과 달리 Notification은 수신자가 항상 관리자로 고정(D-097)이라 주체 컬럼 자체가 필요 없다 — 이 차이를 KDoc에 명시할 것.
...
### `pass/AdminPassService.kt` — 기존 코드 수정 지점 (D-089 선행 검사)
...
fun cancel(passId: Long, request: CancelPassRequest, adminId: Long): PassResponse {
    ...
    // D-089 신설: 대상 이용권으로 잡힌 활성 예약이 있으면 등록 취소를 거부한다.
...
- 헤더 주석에 근거 결정 번호 나열 (`-- V6: ... D-090~D-098 결정에 근거한다`)
