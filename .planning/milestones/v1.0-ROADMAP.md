# Milestone v1.0: 골드레슬링 회원·예약 시스템 BE (M1~M6)

**Status:** ✅ SHIPPED 2026-09-01 (dev 머지 기준 — dev→main 배포는 별도 운영 절차)
**Phases:** 1-6
**Total Plans:** 71 (+ quick task 4건)

## Overview

카카오 로그인 → 관리자 승인 → 이용권(3종) → 예약(즉시 차감/복구) → 배치(미사용 차감) →
운영(출석·공지·알림)까지, "회원이 보는 잔여 횟수 = 실제 사용 가능 횟수" 핵심 가치를
append-only 원장(PassTransaction)과 DB 제약 기반 동시성으로 지키는 백엔드 v1.

- **요구사항**: 44/44 Complete (FOUND 3 · AUTH 6 · MEMBER 4 · PASS 8 · SCHED 3 · RESV 9 · NOTIF 3 · BATCH 4 · ATTEND 2 · NOTICE 2)
- **규모**: 커밋 548 · 프로덕션 12,113 LOC + 테스트 27,178 LOC (Kotlin) · Flyway V1~V12 · openapi 41경로
- **품질**: 테스트 883건 0 failures · 전 phase VERIFICATION passed · 실기동 human-verify 승인
- **타임라인**: 2026-07-27 → 2026-09-01 (36일)
...
**정정**: 위 Progress 표의 "5. 배치 15/16 In Progress"는 갱신 누락된 낡은 행이다 —
실제로는 16/16 Complete (2026-08-16, 재검증 passed 4/4). phase 목록의 체크 표시가 맞다.

**Key Decisions:** 마일스톤 기간 중 D-025~D-151 (127건) — 원본은 `docs/decisions.md`.
굵직한 것: 즉시 차감+append-only 원장(D-001 계열), 조건부 UPDATE 우선·비관적 락 최소(D-021·D-072),
상태 기반 멱등 배치(D-106), 기준일 5종 max(D-105·D-147), ProblemDetail 고정(D-017),
phase→청크 납품(D-084), 보강 수업 불허(D-146), cron 기본 꺼짐 유지(D-121·D-130·D-151).

**Issues Resolved (마일스톤 내 검증→갭 클로저 사이클):**
- Phase 2: CR-01 동시 최초 로그인 경쟁 + WR 5건 (02-12~02-15)
- Phase 5: CR-01 동시 실행 이중 차감, CR-02 소급 차감, CR-04 휴회 우회 경로 (05-10~05-16)
- Phase 6: CR-03 기준일 후보 ① 배선 (06-03), D-01 공지 page/size 500 (quick 260820-cp1)
- 이슈 #12: 휴강 캐스케이드 × 등록취소 경합 500 (quick 260828-g4o, D-145)
- v1 마감: D-136 요일 검증(D-146), WR-06 INACTIVE 차감(D-147), FE 계약 요청 3건(D-148~D-150) — PR #27

**Issues Deferred (→ `.planning/STATE.md` Deferred Items):**
- BE-REQ-001(4xx ProblemDetail 스키마 선언) · BE-REQ-002(`GET /api/admin/me`) ·
  BE-REQ-006(예약 단건 조회) — FE 우회 동작 중, v1.1 백로그
- cron 활성화(운영 배포 후 D-130 절차, D-151) · CR-03 운영 데이터 대조(배포 후 관찰) ·
  WR-02 관리자 로그인 타이밍 부채널(Info, 의도적 제외)
- audit-open 스캐너가 세는 04-HUMAN-UAT 1건은 status passed·미해결 0건 — 실질 열린 항목 아님(인정 처리)

**Tech Debt:** 지난 예약이 영구히 ACTIVE로 남는 모델(BE-REQ-004는 읽기 필터로 해소,
상태 전이 배치는 의도적으로 미도입 — D-150 기각 대안 참조).
