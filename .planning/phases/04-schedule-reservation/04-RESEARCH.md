...
#### 등록 취소(PASS-08)와 예약의 관계 (D-085, policies §1 반영 완료)
...
#### 날짜별 수업(ClassSession) 실체화 (D-090 후보)
...
#### 알림(Notification) — NOTIF-01 범위 (D-093 후보)
...
**What:** "필요할 때 생성"(D-090)이 요구하는 경쟁 안전 get-or-create를, JPA 엔티티 저장 후
...
`ClassSchedule`의 시각을 복사하는 것과 같은 이유(D-090)다.
...
### Pattern 4: 이용권 선택 — 만료 임박순 단일 이용권 (D-087)
...
- **엔티티를 read→mutate→save로 취소/차감 처리** — D-072가 이미 Phase 3에서 겪은 경쟁 버그를
  ...
  같은 원칙을 적용한다
- **`ClassSession` 사전 생성 배치** — D-090이 "배치 인프라는 Phase 5 소관, 배치 미실행 시 예약
...
- **여러 이용권 잔여를 합산해 하나의 예약을 처리** — D-087이 명시적으로 금지("예약 1건 ↔
...
복구 시의 0행은 "복구하지 않는 게 정책"**(D-087 "등록 취소된 이용권은 복구하지 않는다")이라 예외를
던지면 안 된다.
...
복구하지 않는다"가 D-087의 정확한 문구). 예약 생성 경로에서는 0을 예외로 변환한다. 이 두 호출부를
...
컨텍스트가 이미 "변경 = 새 INSERT + 기존 행 취소"로 결정했으므로(D-086), 새 예약은 항상 새 행의
...
D-086이 요구하는 "CANCEL_REFUND(+1)와 RESERVE(−1) 2건 모두 남긴다"는 원장 요구사항과도 충돌한다.
...
CREATE TABLE class_session (
    ...
    start_time TIME NOT NULL,                  -- D-090: 시간표에서 복사, 자기 컬럼으로 보유
    end_time TIME NOT NULL,
...
CREATE TABLE reservation (
    ...
    pass_id BIGINT NOT NULL,                   -- 복구 대상 추적(D-087)
