...
### 동시성 검증 방법 (D-094 후보, 사용자 확정 2026-08-07)
...
- **Phase 3 코드 수정 지점**: `pass/AdminPassService`의 등록 취소 경로에 **활성 예약 선행 검사**를
  추가해야 한다 (D-085). `reservation` 패키지를 참조하게 되므로 의존 방향 주의
...
- `docs/decisions.md` — D-085(등록 취소 선행 조건, policies는 반영 완료)부터
  D-086~D-094(예약 이력 모델, 차감 대상 선택, 중복 예약, 시간표 정의, 세션 실체화,
...
- `docs/policies.md` — §2 시간표에 1:1 예약 가능 타임이 "저녁반·예약제가 열리는 모든 타임"임을
  ...
  문구를 "동시성 통합테스트(ExecutorService)로 검증"으로 정정** (D-094, 사용자 확정)
