    ...
    @Operation(
        ...
        description =
            "**순차** 재실행은 안전하다 — 상태 기반 부족분 계산(D-106)이 이미 존재하는 차감을 " +
                ...
                "**응답이 오지 않아도 재호출하지 않는다** — 이 API는 배치가 끝날 때까지 " +
                "기다렸다 응답하는 동기 호출이라, 프록시·로드밸런서 타임아웃으로 응답을 " +
                "못 받아도 서버에서는 배치가 계속 돌고 있다. 이때 재시도하면 두 실행이 " +
                "겹쳐 위의 이중 차감이 그대로 재현된다. 실제 실행 여부는 batch_execution " +
                "이력으로 확인한다.",
    )
