...
- 2026-09-09 / FE `--primary: oklch(0.8 0.16 85)`(#edb417) · `--primary-foreground: oklch(0.145 0 0)`, `.dark` 동일. 호출부 코드 변경 0건(생성물 `Button` 기본 변형).
...
- 이유: 사용자 판단 — 골드레슬링 브랜드와 카카오 노랑 옆에서 무채색 다크 버튼이 따로 논다. 기존 "골드는 대비가 안 나온다"는 **흰 텍스트 전제**였고,
  다크 텍스트면 10.5:1(AAA). 배경 자체의 흰 배경 대비 1.88:1은 텍스트 라벨이 있어 WCAG 1.4.11 대상이 아니다(D-180과 같은 논리). 새 금지: `text-primary`(`variant="link"`)
