...
## D-180. 카카오 로그인 버튼을 공식 디자인 가이드에 정합시킨다 — 팔레트 변경이 아니라 브랜드 계약값 정정 (D-082 supersede)

- 2026-09-09 / FE PR #53 (Phase 9 09-06). 카카오 공식 로그인 버튼 디자인 가이드(`developers.kakao.com/docs/ko/kakaologin/design-guide`,
  2026-09-09 확인)에 맞춰 `/login` 버튼을 바꾼다: 라벨 `카카오로 로그인하기` → **`카카오 로그인`**(완성형) · 배경 `#FEE500` 유지 ·
  라벨 색 `--kakao-foreground: #191919` → **`rgba(0, 0, 0, 0.85)`**(`#000000` 85%) · radius `rounded-lg`(10px) → **`rounded-[12px]`** ·
  높이 `h-13`(52, 모바일 주 CTA 치수). 폰트(OS 기본 서체)는 앱 전역 Pretendard 유지로 범위 밖.
- 심볼은 **넣지 않는다**: 공식 리소스 묶음이 버튼 전체 PNG만 제공하고 심볼 단독 파일이 없다(FE `09-02-SUMMARY.md`, 사용자 결정).
  크롭·SVG 재작성·제3자 미러는 "형태·비율·색 변경 불가" 보증을 잃어 배제. 가이드가 심볼을 필수로 두므로 비즈앱 심사 지적
  가능성은 인지한 채 결정했다. 라벨 색 합성 실효색 ≈ `#262200` on `#FEE500` = 12.5:1 (AA·AAA 통과).
- 이유: v2 카카오 비즈앱 심사의 버튼 가이드 준수 대비. D-082의 "카카오가 지정한 정확한 브랜드 값"은 배경에는 맞았으나
  전경색은 가이드 값이 아니었다 — 승인본 Out of Scope "색상 팔레트·브랜딩 변경"이나 RULE-05 경로가 아니라 **계약값 정정**이다.
  D-082의 SDK SRI 결정은 그대로 유효하고, 이 번호는 색·라벨·radius·심볼만 supersede 한다.
- 라벨 참조처 12줄/7파일(코드·스토리·감사 하네스 `e2e/audit/routes.ts`·규칙 문서 3)을 한 커밋에서 갱신했다 — 감사 준비 판정이
  라벨로 버튼을 찾기 때문에 쪼개면 재감사가 깨진다.

## D-181. design-system 브랜드 예외 2건 — 버튼 라벨 규칙 · `rounded-[12px]` 임의값

- 2026-09-09 / FE PR #53. (a) "버튼 라벨은 동사+명사" 규칙의 예외: 외부 브랜드 가이드가 라벨을 지정하면 그 문구를 그대로 쓴다
  (`카카오 로그인`). 다른 소셜 로그인이 와도 같은 기준. (b) `rounded-[12px]`를 `tokens.md` §임의값 예외 표에
  `pb-[env(safe-area-inset-bottom)]`과 같은 자격으로 등재 — 허용 범위는 `/login` 카카오 버튼 1곳, 근거는 가이드 radius 12px.
  `rounded-xl`(14px) 금지와 충돌하지 않는다.
- 이유: 규칙 문서만 보는 사람이 위반으로 오인하지 않도록 예외를 기록한다 (CLAUDE.md "규약에 없는 결정은 먼저 문서에 추가한 뒤 사용").
  D-09/D-10 계획의 (c) lucide-only 심볼 예외는 심볼 생략(D-180)으로 **발생하지 않는다** — 3건이 아니라 2건.

## D-185. 하단 고정 CTA 바·Sheet 푸터의 위쪽 경계선(`border-t`)을 두지 않는다

- 2026-09-09 / FE PR #53 리뷰에서 사용자가 "푸터 버튼 위 밑줄을 전부 빼 달라"고 했다. `ScreenActionBar`(모바일 하단 고정 CTA)와
  `ResponsiveDialog` Sheet 경로 푸터에서 `border-t`·`sm:border-t-0`을 제거한다. 배경색(`bg-background`·`bg-popover`)만으로
  sticky 겹침을 가리고 그림자는 여전히 쓰지 않는다. 바 실측 높이가 77 → **76**(py-3 24 + h-13 52)이 되어
  `html { scroll-padding-bottom }` 상수(D-16 짝)와 `ScreenActionBar.stories.tsx` 단언을 같이 76으로 맞췄다.
- 범위(같은 날 "모든 페이지 다 적용했어?" 확인으로 확장): 셸 하단 탭 바(`MemberShell`·`AdminShell` `<nav>`)의 hairline,
  `AdminShell` 사이드바 로그아웃 위 선, 그리고 shadcn 생성물 `DialogFooter`·`AlertDialogFooter`의 기본
  `bg-muted/50 border-t`(회색 띠 + 선)까지 전부 제거한다. 생성물은 고치지 않고 소비처(`ResponsiveDialog` Dialog 경로 ·
  `ConfirmDialog`)에서 `border-t-0 bg-transparent`로 병합 오버라이드한다. 남는 구획 수단은 배경색과 `mt-auto`뿐이다.
- 이유: 콘텐츠가 짧은 화면(로그인)에서 경계선이 본문과 CTA를 두 구획으로 갈라 보이게 하고, 그 위 빈 여백이
  "줄바꿈 영역"처럼 읽힌다. "구획은 border로 만든다"(CLAUDE.md UI 기본선)는 목록·섹션 구획의 규칙이고, 화면 하단에
  붙는 액션 바는 배경색으로 이미 구획된다.
- 기각 대안: 그림자로 대체 — 정적 요소 shadow 금지(CLAUDE.md UI 기본선 · design-system anti-patterns.md)와 충돌.

## D-186. 주 액션 색을 무채색 다크에서 **브랜드 골드 + 다크 텍스트**로 바꾼다

- 2026-09-09 / FE `--primary: oklch(0.8 0.16 85)`(#edb417) · `--primary-foreground: oklch(0.145 0 0)`. `.dark`도 같은 값.
  design-system SKILL.md·tokens.md·anti-patterns.md·CLAUDE.md UI 기본선의 "주 액션 색은 무채색 다크 하나" 문구를 전부 갱신했다.
  `bg-primary text-primary-foreground`는 생성물 `Button` 기본 변형이라 호출부 코드 변경 0건, 클래스 단언 스토리 무수정.
- 이유: 사용자 판단 — 브랜드가 골드레슬링이고 카카오 버튼이 노랑 계열이라 무채색 다크 버튼이 화면에서 따로 논다.
  기존 결정("골드는 흰 배경에서 대비가 안 나온다", tokens.md 표)은 **흰 텍스트를 얹는 전제**였다. 카카오 버튼처럼
  다크 텍스트를 얹으면 #edb417 위 oklch(0.145 0 0) = **10.50:1**(AAA)이고, 배경 자체의 흰 배경 대비 1.88:1은 안에 텍스트
  라벨이 있어 WCAG 1.4.11 대상이 아니다 — 카카오 토큰(D-082/D-180)과 같은 논리다.
- 골드 램프: `--primary`(0.80, 주 액션 배경) > `--brand`(0.65, 얇은 인디케이터 — 흰 배경 3.33:1) > `--brand-strong`(0.55, 흰 배경 위
  텍스트 5.00:1). 표식 토큰을 `--primary`로 합치지 않는 이유는 얇은 선·텍스트는 자기 색으로 3:1/4.5:1을 맞춰야 하기 때문이다.
- 새 금지: `text-primary`(골드 텍스트 on white 1.88:1) — `Button`·`Badge` `variant="link"` 사용 금지(현재 0건).
  카카오 버튼(#FEE500)과의 구분: 채도·명도가 달라 같은 톤 계열이면서 다른 색으로 읽힌다(스와치 실측). 카카오 값을 그대로
  쓰지 않는 이유는 카카오 로그인 버튼이 카카오 브랜드 자산으로 식별되어야 하기 때문이다.
- 기각 대안: ① 카카오 `#FEE500` 그대로 — 카카오 버튼과 구별 불가. ② 기존 `--brand` #c47d04 배경 — 다크 텍스트 5.9:1로 가능하나
  스와치에서 앰버/갈색으로 읽혀 "노란색 계열" 요구와 어긋남. ③ 골드 배경 + 흰 텍스트 — 1.88:1 불가.
- 번호 주의: D-178·D-179·D-182~D-184는 FE Phase 9 09-08이 예약한 번호라 비워 두고 채번했다 (D-180·D-181은 PR #53 리뷰 반영으로 같은 날 먼저 기록).
